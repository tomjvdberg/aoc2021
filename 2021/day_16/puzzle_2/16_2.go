...
func (s *knownLengthSegment) getValue(typeId int64) []int64 {
	return []int64{s.bits.Int64()}
}
func (s *knownLengthSubpacketSegment) getValue(typeId int64) []int64 {
	vs := []int64{}
	for _, p := range s.packets {
		vs = append(vs, applyRules(p.typeId, p.literalValues))
	}

	return []int64{applyRules(typeId, vs)}
}
func (s *countSubpacketSegment) getValue(typeId int64) []int64 {
	vs := []int64{}
	for _, p := range s.packets {
		vs = append(vs, applyRules(p.typeId, p.literalValues))
	}
...
func (p *packet) pushBit(b uint, versions *[]int64) bool {
	...
	case VERSION:
		p.version = p.curSegment.getValue(p.typeId)[0]
		*versions = append(*versions, p.version)
		p.curSegment = newTypeIdSegment()
	case TYPE_ID:
		p.typeId = p.curSegment.getValue(p.typeId)[0]
		if p.typeId == LITERAL {
	...
	case LENGTH_TYPE_ID:
		if p.curSegment.getValue(p.typeId)[0] == 0 {
	...
	case TOTAL_LENGTH_OF_SUBPACKETS:
		totalLengthOfSubpackets := p.curSegment.getValue(p.typeId)[0]
		p.curSegment = newKnownLengthSubpacketSegment(totalLengthOfSubpackets)
	case COUNT_OF_SUBPACKETS:
		countOfSubpackets := p.curSegment.getValue(p.typeId)[0]
		p.curSegment = newCountSubpacketSegment(countOfSubpackets)
	case KNOWN_LENGTH_SUBPACKETS:
		p.literalValues = append(p.literalValues, p.curSegment.getValue(p.typeId)...)
		p.value = applyRules(p.typeId, p.literalValues)
		return true
...
func applyRules(typeId int64, literalValues []int64) int64 {
	val := int64(0)
	switch typeId {
	case SUM:
		for _, litVal := range literalValues {
			val += litVal
		}
	case PRODUCT:
		val = literalValues[0]
		for _, litVal := range literalValues[1:] {
			val *= litVal
		}
	case MINIMUM:
		val = literalValues[0]
	...
	case LITERAL:
		val = valueOfLiteralsConcatenated(literalValues)
	case GREATER_THAN:
...
func valueOfLiteralsConcatenated(literalValues []int64) int64 {
	litPointer := (len(literalValues) * 4) - 1 // the value bit length
	x := big.NewInt(0)
	for _, p := range literalValues {
		pv := big.NewInt(p)
		for i := 3; i > -1; i-- {
